...
export const <%= nombreSoloMayusculas %><%= esFormulario ? '' : '_BUSQUEDA' %>_CAMPO_LATITUD_LONGITUD_<%= nombreCampoSoloMayusculas %>: (
    <% if(esFormulario) { %>
...
const latitudLongitudInicial = valorCampo ? valorCampo : {latitud: 0, longitud: 0};

  return {
    tipoCampoHtml: 'latitudLongitud',
      <% if(esFormulario) { %>
...
<% if(internacionalizar) { %>
  ...
  <% if(internacionalizar) { %>
    ...
    latitudLongitud: {
      latitudLongitud: latitudLongitudInicial,
      // device: undefined,
      mostrarLatitudLongitud: true,
      opcionesGoogleMaps: {
        bounds: {
          north: latitudLongitudInicial.latitud + 0.5,
          south: latitudLongitudInicial.latitud - 0.5,
          east: latitudLongitudInicial.longitud + 0.5,
          west: latitudLongitudInicial.longitud - 0.5,
        },
        componentRestrictions: {country: 'ec'},
        fields: ['geometry', 'name'],
        origin: latitudLongitudInicial,
        strictBounds: false,
        types: [],
      },
    },
